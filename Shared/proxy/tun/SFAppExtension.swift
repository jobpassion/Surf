...
//  Surf
//
//  Created by networkextension on 8/2/16.
...
extension SFRequestInfo {
    func detailString() ->NSAttributedString {
        ...
        //let range = NSMakeRange(r?.startIndex, )
            ...
            s.addAttributes([NSAttributedString.Key.backgroundColor:UIColor.red,NSAttributedString.Key.foregroundColor:UIColor.white], range: ra)
            
            s.addAttributes([NSAttributedString.Key.foregroundColor:color], range: r)
            
            r = (x as NSString).range(of: modeString)
            
            s.addAttributes([NSAttributedString.Key.foregroundColor:UIColor.white,NSAttributedString.Key.backgroundColor:UIColor.blue], range: r)
            if !t.isEmpty{
                let scode =  UIColor.init(red: 0.36, green: 0.65, blue: 0.76, alpha: 1.0)
                let srange = (x as NSString).range(of: t)
                s.addAttributes([NSAttributedString.Key.foregroundColor:scode], range: srange)
            }
            ...
            s.addAttributes([NSAttributedString.Key.foregroundColor:NSColor.white,NSAttributedString.Key.backgroundColor:NSColor.blue], range: r)
            #endif
        return s
