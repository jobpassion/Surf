...
class UDPServerConnection: Connection {
    ...
    func createSocketWithAddressFamilyFromAddress(address: String) -> Bool {
		...
		dispatch_source_set_cancel_handler(newResponseSource) {
			AxLogger.log("closing udp socket for connection \(self.identifier)")
		...
		dispatch_source_set_event_handler(newResponseSource) {
			...
			guard bytesRead >= 0 else {
				if let errorString = String(UTF8String: strerror(errno)) {
					AxLogger.log("recvfrom failed: \(errorString)")
				}
			...
			guard bytesRead > 0 else {
				AxLogger.log("recvfrom returned EOF")
				self.closeConnection(.All)
				return
			}

			guard let endpoint = withUnsafePointer(&socketAddress, { self.getEndpointFromSocketAddress(UnsafePointer($0)) }) else {
				AxLogger.log("Failed to get the address and port from the socket address received from recvfrom")
				self.closeConnection(.All)
				return
			}

			let responseDatagram = NSData(bytes: UnsafePointer<Void>(response), length: bytesRead)
			AxLogger.log("UDP connection id \(self.identifier) received = \(bytesRead) bytes from host = \(endpoint.host) port = \(endpoint.port)")
    ...
    override func sendDataWithEndPoint(data: NSData, host: String, port: Int) {
		...
		if responseSource == nil {
			guard createSocketWithAddressFamilyFromAddress(host) else {
				AxLogger.log("UDP ServerConnection initialization failed.")
				return
			}
		}
		...
		switch addressFamily {
			case AF_INET:
				...
				guard serverAddress.setFromString(host) else {
					AxLogger.log("Failed to convert \(host) into an IPv4 address")
					return
				}
			...
			case AF_INET6:
				...
				guard serverAddress.setFromString(host) else {
					AxLogger.log("Failed to convert \(host) into an IPv6 address")
					return
				}
				serverAddress.setPort(port)
		...
		guard sent > 0 else {
			if let errorString = String(UTF8String: strerror(errno)) {
				AxLogger.log("UDP connection id \(identifier) failed to send data to host = \(host) port \(port). error = \(errorString)")
			}
			closeConnection(.All)
			return
		}
		...
		if sent == data.length {
			// Success
			AxLogger.log("UDP connection id \(identifier) sent \(data.length) bytes to host = \(host) port \(port)")
		}
    }
